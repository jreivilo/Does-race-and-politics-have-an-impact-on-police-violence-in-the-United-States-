...
To get started, we had to rearrange the data set in order to retain only the variables and observations we needed. This data set was huge, we had to concentrate on the essential : Moving from 53 variables to 12. Then we did basic modifications such as changing variable names, filtering the variables with NAs, changing some observations to make the understanding clearer. 
...
kable(white_state_killings) %>% 
  kable_styling(fixed_thead = T, full_width = F, position = "left") %>% 
  column_spec(10, bold = T, italic = T, background = "gainsboro") %>%
  column_spec(10, bold = T, italic = T, background = "darkseagreen") %>%
...
This represent people who aren't Black who were killed despite showing no apparent reason to be. As a result, 168 people were killed in these circumstances.
...
kable(summary(killings_per_year)) %>% 
  kable_styling(fixed_thead = T, full_width = F, position = "left") %>% 
  row_spec(4, bold = T, italic = T, background = "gainsboro") %>%
  row_spec(0, bold = T, italic = T, color = "white", background = "brown") 
...
police_killings %>% 
  ggplot(mapping=aes(x=reorder(race,race,function(x)-length(x)),fill=race)) +
    ...
    geom_label(aes(x= race,
               label=..count..),
               stat='count') +
    labs(title="Population killed by the police from 2013 to 2021",  x = "Race",
          y= "Number people killed")

ggplot(police_killings) +
  geom_bar(mapping=aes(x=race,fill=gender), position="fill") + 
  labs(title="Gender killed by the police from 2013 to 2021")
```
...
The proportion of women killed is very little compared to men. However we can observe that White, Black and Hispanic women are the most affected. 
...
kbs %>%
ggplot(.)+
  geom_point(mapping=aes(x=reorder(`State Abbreviation`,-`% Black`),y=`% Black`), color="brown") + 
    geom_point(mapping=aes(x=reorder(`State Abbreviation`,-`% Black victims`),y=`% Black victims`)) +
