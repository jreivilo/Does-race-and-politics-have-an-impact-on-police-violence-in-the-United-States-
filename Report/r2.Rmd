...
These are the ones we are going to link with other datasets to run our hypothesis. 
...
```{r, echo = FALSE, message = FALSE, warning=FALSE}
police_killings %>% ggplot() +
coord_flip() + 
geom_bar(mapping=aes(x=race,fill=race)) +
geom_label(aes(x= race,
                label=..count..),
            stat='count') +
labs(title="Population killed by the police from 2013 to 2021")
...
kbs %>%
ggplot(.)+
  geom_point(mapping=aes(x=reorder(`State Abbreviation`,-`% African-American`),y=`% African-American`), color="brown") + 
