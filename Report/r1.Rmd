...
kable(Gender_Composition[c(1,2,6)]) %>% 
  ...
  scroll_box(height = "400px", width = "28%")

```


We only kept this 3 variables to have a claer vision of the evolution among time of the gender diversity in the police. 
...
kable(RAE[c(1,2,3,7 )]) %>% 
  ...
  scroll_box(height = "400px", width = "55%")
```
...
The huge share that black people represent in the violent crime could explain why they are overkilled compared to their population. In 2016, black people represent 13.3% of the population but 44.2% of violent crime. The ratio is 3.3. On the other side, they represent approximately 25% of people getting shot every year. The ratio in this the case is 1.9.
...
kable(election_percent) %>% kable_styling(fixed_thead = T, full_width = F, position = "left") %>% 
  ...
  scroll_box(height = "350px", width = "70%")

```


Almost every state got more democrat vote in 2020 than in 2016. This led to the elction of a democrat president in 2020.
...
After a rise between 2014 and 2016, the crime rate is dropping every year until 2019. 
