...
The first model we tried was with all the ovservation but is was not a good match at all. Removing the outliers as showed in the paper slightly improved the model but not significatively. The goal was to find a variable that could explain this disparitie in order to influence it in the real word to lowered the disparity rate. With this model, we are not able to do it. However, the previous model and the correlation table gave us some clues; the best way to lowered the disparities between black and white is to lowered the crime rate for black people and to understand what make democrat state less likely to shoot offender. It might not lower the disparities (as it is not proved by the model) but it will al least lower the number of black getting shot every year. 
